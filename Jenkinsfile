...
'''
    ...
    /* =================================================
       🔍 DEBUG BLOCK (RUNS EVEN IF PIPELINE FAILS)
       ONLY ADDITION – NO OTHER CHANGES
    ================================================= */
    post {
        always {
            container('kubectl') {
                sh '''
                    echo "================ POD STATUS ================"
                    kubectl get pods -n 2401193 -o wide || true

                    echo "================ SERVICE STATUS ================"
                    kubectl get svc -n 2401193 || true

                    echo "================ ENDPOINTS ================"
                    kubectl get endpoints -n 2401193 || true

                    echo "================ BACKEND POD LOGS ================"
                    BACKEND_POD=$(kubectl get pods -n 2401193 -l app=mern-backend -o jsonpath="{.items[0].metadata.name}" 2>/dev/null)
                    kubectl logs $BACKEND_POD -n 2401193 --tail=200 || true

                    echo "================ FRONTEND POD LOGS ================"
                    FRONTEND_POD=$(kubectl get pods -n 2401193 -l app=mern-frontend -o jsonpath="{.items[0].metadata.name}" 2>/dev/null)
                    kubectl logs $FRONTEND_POD -n 2401193 --tail=200 || true
